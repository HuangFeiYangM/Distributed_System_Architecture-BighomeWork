-- 单库方案：与 README 及各服务 application.yml 中 jdbc:mysql://.../canteen 一致
...
CREATE TABLE orders (
    ...
    pickup_no       VARCHAR(16) COMMENT '取餐号（叫号用）',
    pickup_code     VARCHAR(16) UNIQUE COMMENT '取餐码（核销用）',
...
CREATE TABLE order_item (
    ...
    dish_id         BIGINT UNSIGNED NOT NULL COMMENT '菜品ID',
    menu_dish_id    BIGINT UNSIGNED COMMENT '当日菜单菜品ID',
...
CREATE TABLE canteen_window (
    ...
    pickup_prefix   VARCHAR(8) DEFAULT 'A' COMMENT '取餐号前缀',
    create_time     DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time     DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted         TINYINT DEFAULT 0 COMMENT '逻辑删除',
    INDEX idx_merchant (merchant_id),
